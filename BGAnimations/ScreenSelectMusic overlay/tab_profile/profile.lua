        ...
        UpdateRankingMessageCommand = function(self)
            if rankingSkillset > 1 and update and not recentactive then
                if not showOnline then
                    ths = SCOREMAN:GetTopSSRHighScoreForGame(i + (scoresperpage * (rankingPage - 1)),
                        ms.SkillSets[rankingSkillset])
                    if ths then
                        self:visible(true)
                        ck = ths:GetChartKey()
...
local function rankingButton(i)
    local t = Def.ActorFrame {
        ...
        UIElements.QuadButton(1, 1) .. {
            InitCommand = function(self)
                self:zoomto(rankingTitleSpacing, 26):diffuse(getMainColor("frames")):diffusealpha(0)
            end,
...
t[#t + 1] = Def.ActorFrame {
    ...
    Def.ActorFrame {
        ...
        UIElements.QuadButton(1, 1) .. {
            InitCommand = function(self)
                self:zoomto(rankingTitleSpacing, 26):diffusealpha(0):diffuse(getMainColor("frames"))
            end,
            SetCommand = function(self)
                if not showOnline then
                    self:diffusealpha(1)
    ...
    Def.ActorFrame {
        ...
        UIElements.QuadButton(1, 1) .. {
            InitCommand = function(self)
                self:zoomto(rankingTitleSpacing, 26):diffusealpha(0)
                if DLMAN:IsLoggedIn() then
...
r[#r + 1] = Def.ActorFrame {
    ...
    UIElements.QuadButton(1, 1) .. {
        InitCommand = function(self)
            self:xy(capWideScale(300, 336.25), -8.5):zoomto(40, 20):halign(0):valign(0):diffuse(getMainColor("frames"))
                :diffusealpha(0)
        end,
        MouseDownCommand = function(self, params)
    ...
    LoadFont("artist 16px") .. {
        Name = "NextP",
        InitCommand = function(self)
            self:x(capWideScale(304.25, 340)):halign(0):zoom(0.3):diffuse(getMainColor("positive")):settext(
                translated_info["NextPage"])
        end,
    },
    UIElements.QuadButton(1, 1) .. {
        InitCommand = function(self)
            self:xy(-2, -8.5):zoomto(65, 20):halign(0):valign(0):diffuse(getMainColor("frames")):diffusealpha(0)
        end,
        MouseDownCommand = function(self, params)
            if params.event == "DeviceButton_left mouse button" then
...
local function littlebits(i)
    local t = Def.ActorFrame {
        InitCommand = function(self)
            self:xy(frameX + capWideScale(28, 45), frameY - 80)
            self:zoom(0.95)
        end,
        UpdateRankingMessageCommand = function(self)
            if rankingSkillset == 1 and update and not recentactive then
...
local profilebuttons = Def.ActorFrame {
    ...
    UIElements.TextToolTip(1, 1, "artist 16px") .. {
        InitCommand = function(self)
