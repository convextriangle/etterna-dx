...
local lowDensityColor = color("#AAAAAA")
local highDensityColor = color("#232323")
            ...
            if song and isOver(bg) then
                ...
                if stepsinuse ~= nil then
                    ...
                    if self.npsVector ~= nil and #self.npsVector > 0 then
                        ...
                        local hoveredIndex = clamp(math.ceil(self.finalNPSVectorIndex * percent),
                            math.min(1, self.finalNPSVectorIndex), self.finalNPSVectorIndex)
                        ...
                        stro = string.format("%s\n%d %s\n%d %s", postext, hoveredNPS, translations["NPS"], bpm,
                            translations["BPM"])
                    end
        ...
        for density = 1, ncol do
            for column = 1, numberOfColumns do
                if graphVectors[density][column] > 0 then
                    ...
                    makeABar(verts, math.min(column * columnWidth, sizing.Width), yOffset, columnWidth,
                        graphVectors[density][column] * 2 * heightScale, barColor)
...
t[#t + 1] = LoadFont("artist 16px") .. {
    ...
    UpdateSizingCommand = function(self)
        self:finishtweening()
        self:smooth(resizeAnimationSeconds)
        local zs = (sizing.TextSize or 1) / 2
        self:zoom(zs)
